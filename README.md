...
4. **Sécurisation des identifiants (BCrypt via NuGet) :** Le script télécharge le package `BCrypt.Net-Next 4.2.0` depuis NuGet, charge la DLL en mémoire (sans installation, sans fichier résiduel), et génère le hash BCrypt de votre mot de passe localement. <ins>**Le hash est généré localement sur votre machine : aucun mot de passe ne circule en clair.**</ins>
...
4. **Credential Security (BCrypt via NuGet):** The script downloads the `BCrypt.Net-Next 4.2.0` package from NuGet, loads the DLL in memory (no installation, no leftover files), and generates the BCrypt hash of your password locally. <ins>**The hash is generated locally on your machine: no plaintext password ever leaves your system.**</ins>
